...
public class AsyncPacketWriter implements Closeable {
    ...
    private final IoArgsFinal args = new IoArgsFinal();
     ...
     * @return IoArgs
     */
    synchronized IoArgsFinal takeIoArgs() {
     ...
     * @param args IoArgs
     */
    synchronized void consumeIoArgs(IoArgsFinal args) {
        IoArgsFinal argsFinal=args;
        if (frameTemp == null) {
            Frame temp;
            do {
                // 还有未消费数据，则重复构建帧
            ...
            try {
                if (currentFrame.handle(args)) {
                    ...
                    if (currentFrame instanceof ReceiveHeaderFrame) {
                        ...
                        ReceivedPacket packet = provider.takePacket(headerFrame.getPacketType(),
     ...
     * @return 返回新的帧
     */
    private Frame buildNewFrame(IoArgsFinal args) {
    ...
    private void cancelReceivePacket(short identifier) {
        synchronized (packetMap) {
            ...
            if (model != null) {
                ReceivedPacket packet = model.packet;
     ...
     * @param packet     Packet
     */
    private void appendNewPacket(short identifier, ReceivedPacket packet) {
        synchronized (packetMap) {
    ...
    interface PacketProvider {
         ...
         */
        ReceivedPacket takePacket(byte type, long length, byte[] headerInfo);
         ...
         */
        void completedPacket(ReceivedPacket packet, boolean isSucceed);
    ...
    static class PacketModel {
        final ReceivedPacket packet;
        final WritableByteChannel channel;
        volatile long unreceivedLength;

        PacketModel(ReceivedPacket<?, ?> packet) {
            this.packet = packet;
